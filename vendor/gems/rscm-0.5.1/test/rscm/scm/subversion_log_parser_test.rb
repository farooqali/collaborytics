...
EOF
    ...
    def OBSOLETE_SINCE_URL_ARG_WILL_FILTER_test_should_filter_out_unwanted_entries
...
EOF
    ...
    def test_should_find_relative_path_by_matching
      ...
      #obsolete since url arg will filter out
...
EOF
    ...
    def test_no_prefix_log
      parser = SubversionLogParser.new(StringIO.new(NO_PREFIX_LOG), "svn://mooky/funny", nil, nil, "")
      revisions = parser.parse_revisions
...
NO_AUTHOR_LOG = <<-EOF
------------------------------------------------------------------------
r407118 | (no author) | (no date) | 1 line


------------------------------------------------------------------------
r407119 | sebor | 2006-05-16 20:27:28 -0500 (Tue, 16 May 2006) | 5 lines
Changed paths:
   M /incubator/stdcxx/trunk/tests/containers/23.vector.modifiers.cpp
...
EOF
    ...
    # sometimes svn logs have no authors or dates. this is probably because of
    # migrating from a different scm such as cvs
